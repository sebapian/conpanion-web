...
interface FileViewerProps {
  ...
  directFileUrls?: Array<{ url: string; type?: AttachmentFileType; name?: string; size?: number; isImage?: boolean; isPdf?: boolean }>;
  className?: string;
...
const determineFileType = (file: { url: string; isImage?: boolean; isPdf?: boolean }): AttachmentFileType => {
...
export function FileViewer({ attachmentIds = [], directFileUrls = [], className, viewerType = 'grid' }: FileViewerProps) {
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [fileUrls, setFileUrls] = useState<Array<{ id: string; url: string; type: AttachmentFileType; name: string; size: number }>>([]);
  ...
  const renderGridView = () => (
    <div className={cn(`grid gap-2 ${fileUrls.length === 1 ? 'grid-cols-1' : fileUrls.length === 2 ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-3'}`, className)}>
      {fileUrls.map((file, index) => (
        <div 
          key={file.id} 
          className={`relative cursor-pointer overflow-hidden rounded-md border bg-muted ${
            fileUrls.length === 1 ? 'aspect-video w-full max-w-md mx-auto' : 'aspect-square'
          }`}
        ...
        >
          {file.type === 'image' ? (
            ...
            <>
              <Image 
                ...
                sizes={fileUrls.length === 1 ? "(max-width: 768px) 100vw, 450px" : "(max-width: 768px) 50vw, 33vw"}
            ...
            <div className="flex h-full w-full flex-col items-center justify-center p-4">
              ...
              <p className="mt-2 text-center text-sm truncate max-w-full">{file.name}</p>
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center opacity-0 transition-opacity hover:bg-black/30 hover:opacity-100">
            ...
            {file.type === 'image' && (
              <p className="text-center text-sm text-white px-2 mt-2 max-w-full truncate">{file.name}</p>
            )}
          </div>
        </div>
      ))}
                ...
                <>
                  {fileUrls[currentFileIndex].type === 'image' ? (
                    ...
                    <div className="flex h-full w-full flex-col items-center justify-center">
                      <div className="bg-white/10 p-6 rounded-md flex flex-col items-center">
                        {fileTypeIcons[fileUrls[currentFileIndex].type]}
                        <p className="mt-4 text-center">{fileUrls[currentFileIndex].name}</p>
                        <p className="text-sm text-gray-400">{formatFileSize(fileUrls[currentFileIndex].size)}</p>
                        <Button 
                          className="mt-4" 
