...
function EntriesPageContent({ entryId }: { entryId: string | null }) {
  ...
  const filterEntries = (
    ...
    if (currentSearchTerm) {
      ...
      if (trimmedSearch) { // Only filter if search term is not just whitespace
      ...
      case "question":
        return (
          <div className="space-y-2">
            ...
            />
            {hasError && <p id={`error-${itemId}`} className="text-sm text-red-500 font-medium">{formErrors[itemId]}</p>}
          </div>
        );
            ...
            <div className="space-y-2">
              {item.options?.map((option, index) => {
                ...
                const answerValues = Array.isArray(currentValues) 
                  ? currentValues.map((a: any) => typeof a === 'object' && a !== null ? 
                      (a.value !== undefined ? a.value : JSON.stringify(a)) : a)
                  : [];
      ...
      case "photo":
        return (
          <div className="space-y-2">
            ...
            <div className="border-2 border-dashed rounded-md p-6 text-center border-muted">
                ...
                >
                  ...
                  <TableCell>
                    <Badge variant="outline" className={
                      entry.approval_status === 'approved' ? 'bg-green-50 text-green-700 border-green-200' :
                      entry.approval_status === 'declined' ? 'bg-red-50 text-red-700 border-red-200' : 
                      entry.approval_status === 'submitted' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                      entry.approval_status === 'revision_requested' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                      ''
                    }>
                      {entry.approval_status || 'N/A'}
                    </Badge>
                  </TableCell>
                  <TableCell>{entry.created_at ? format(new Date(entry.created_at), "PPP") : 'N/A'}</TableCell>
              ...
              <div className="p-6 border-b">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    {isEditMode ? (
                      <div className="mb-4">
                        <Label htmlFor="edit-entry-name" className="text-base font-semibold mb-2 block">
                      ...
                      <>
                        <div className="flex items-center gap-2 mb-2">
                          ...
                          {approvalStatus && (
                            <Badge variant="outline" className={
                              ...
                              approvalStatus === 'declined' ? 'bg-red-50 text-red-700 border-red-200' : 
                              approvalStatus === 'submitted' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                              approvalStatus === 'revision_requested' ? 'bg-yellow-50 text-yellow-700 border-yellow-200' :
                              ''
                            }>
                              {approvalStatus}
                            </Badge>
                        ...
                        <div className="flex flex-col gap-1">
                          ...
                          <p className="text-sm text-muted-foreground">
                            Date: {entryDetail.entry.created_at ? format(new Date(entryDetail.entry.created_at), "PPP") : 'N/A'}
                          </p>
                        </div>
                      </>
                    )}
                ...
                <div className="flex justify-end gap-2">
                  {isEditMode ? (
                    ...
                    <>
                      ...
                      {approvalStatus && (approvalStatus === 'draft' || approvalStatus === 'revision_requested') && (
                        <Button
                          variant="default"
